...
create_demographic_visualizations <- function(data) {
  ...
  provider_plot <- ggplot(data, aes(x = .data[[survey_categories$Demographics$Provider]])) +
    geom_bar() +
    theme_minimal() +
    labs(title = "Distribution of Service Providers", x = "Service Provider", y = "Count")
  
  # Bar Chart for Services Contracted For
  services_plot <- ggplot(data, aes(x = .data[[survey_categories$Demographics$Services]])) +
    geom_bar() +
    theme_minimal() +
    labs(title = "Services Contracted For", x = "Services", y = "Count")
  
  # Bar Chart for Primary Territories
  territory_plot <- ggplot(data, aes(x = .data[[survey_categories$Demographics$Territory]])) +
    geom_bar() +
    theme_minimal() +
