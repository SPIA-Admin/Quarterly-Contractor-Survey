...
AnecdotalInsights = list(
  ...
  SuccessStory = list(
    ...
    viz_type = "wordcloud",
    sql_query = "SELECT \"Describe a recent success story or a significant milestone your company achieved.\" AS Answer FROM responses where Answer <> ''"
  ),
  SuggestionForImprovement = list(
    question = "If you could suggest one change to improve contractor relations, what would it be?",
    viz_type = "wordcloud",
    sql_query = "SELECT \"If you could suggest one change to improve contractor relations, what would it be?\" AS Answer FROM responses where Answer <> ''"
  ),
  IndustryChangeImpact = list(
    ...
    viz_type = "wordcloud",
    sql_query = "SELECT \"Are there any upcoming industry changes or trends that you believe will impact your business positively or negatively in the next year?\" AS Answer FROM responses where Answer <> ''"
  ),
  RelationshipWithCompany = list(
    ...
    viz_type = "wordcloud",
    sql_query = "SELECT \"Share an experience that exemplifies your relationship with the company your contract is with.\" AS Answer FROM responses where Answer <> ''"
  ),
  ChallengesAndRewards = list(
    ...
    viz_type = "wordcloud",
    sql_query = "SELECT \"What's one thing you wish outsiders knew about the challenges and rewards of being a Service Provider contractor?\" AS Answer FROM responses where Answer <> ''"
  )
)
)
